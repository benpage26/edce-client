...
def checkInteractive():
    ...
    try:
        ...
        ok = False
    if not ok:
        raise edce.error.Error(
            'Cannot run in non-interactive mode if either the username or password is not set. Set edce.globals.interactive=True, or run setup.py and enter a username and password')
...
def submitMarket(s):
    if edce.globals.debug:
        print(">>>>>>>>>>>>>>>> submitMarket")
    url = edce.config.getString('urls', 'url_market')
    r = s.get(url, verify=True)
    if r.status_code == requests.codes.ok:
        s.cookies.save()
        return r.text
    else:
        errstr = "Error: submitMarket FAIL %s" % r.status_code
        if edce.globals.debug and edce.globals.interactive:
            print(errstr)
        raise edce.error.ErrorProfile(errstr)
...
def askVerificationCode():
    code_raw = input(
        "Verification Code required. You will receive an email from Frontier with a code in it. Enter it here: ")
    code = code_raw.strip()
    return code
...
def performQuery(s=None, verificationCodeSupplyFn=askVerificationCode):
    ...
    if delta <= minimumDelay:
        errstr = "Error: To avoid overloading the FD servers, you must wait %s seconds before your next query" % math.floor(
            minimumDelay - delta)
        if edce.globals.debug and edce.globals.interactive:
            print(errstr)
